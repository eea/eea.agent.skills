# EEA Node.js/Volto add-on pipeline example — volto-addon-template

Source: `https://github.com/eea/volto-addon-template/blob/master/Jenkinsfile`
...
A Volto (React/Plone frontend) add-on that needs to test against multiple
Volto core versions and run a full Cypress integration suite against a real
Plone backend. This is the most elaborate of the EEA example pipelines —
reach for it when a project needs any of its distinctive patterns, not as a
default template:

- Tests two Volto versions in parallel (`Volto 18-yarn` / current, and
  `Volto 17` / previous), each building its own test image via
  `--build-arg=VOLTO_VERSION=...` and running unit + Cypress integration
  tests independently — a matrix-testing pattern worth reusing whenever a
  project must stay compatible with more than one version of a shared
  platform/framework.
- A `Check if testing needed` stage short-circuits the entire test matrix
  (`env.SKIP_TESTS = 'yes'`) when a dedicated `eeacms/gitflow` check script
  decides the change doesn't need it — useful when a large test suite is
  expensive enough that skipping it for certain change types (e.g. docs-only)
...
- Integration tests spin up a real backend (`eeacms/plone-backend`) and a
  ...
  failure evidence (`for file in ...; if [ 0 failures ]; then rm the video
  fi`) — worth reusing whenever integration test artifacts (especially
  video) would otherwise balloon storage for a mostly-passing suite.
- A dedicated `SonarQube compare to master` stage runs a script
  (`checkSonarqubemasterV2.sh` via `eeacms/gitflow`) that fails the PR if
  the `develop`/PR branch's SonarQube metrics regress against `master`
  specifically for new bugs — a stricter, comparison-based quality gate
  distinct from this skill's default absolute-threshold gates.
- SonarQube LCOV coverage import needs a path-rewrite step
  (`sed -i "s#/app/src/addons/${GIT_NAME}/##g" ...`) because the coverage
  file was generated inside a container at a different path than the Sonar
  scan's project base dir — the same class of path-mismatch problem
  covered in the EEA Docker-outside-of-Docker constraint, solved here with
  `sed` rather than avoiding the mismatch.
